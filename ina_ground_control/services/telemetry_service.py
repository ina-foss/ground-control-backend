...
class TelemetryService:
    ...
    def __init__(self, application: FastAPI, sql_alchemy_engine):
        ...
        self.resource = Resource.create(
            {
                ...
                ResourceAttributes.K8S_NAMESPACE_NAME: os.getenv(
                    "KUBERNETES_NAMESPACE", "unknown-namespace"
                ),
                ResourceAttributes.K8S_POD_NAME: os.getenv(
                    "KUBERNETES_POD_NAME", "unknown-pod"
                ),
                ResourceAttributes.K8S_POD_UID: os.getenv(
                    "KUBERNETES_POD_UID", "unknown-node"
                ),
                ResourceAttributes.K8S_NODE_NAME: os.getenv(
                    "KUBE_NODE_NAME", "unknown-node"
                ),
            }
        )
        self._setup_metrics()
        ...
        FastAPIInstrumentor.instrument_app(
            self.application, excluded_urls="/management/metrics"
        )
    ...
    def _setup_metrics(self):
        ...
        meter_provider = MeterProvider(
            metric_readers=[prometheus_reader], resource=self.resource
        )
