"""Module Middleware for monitoring FastAPI applications using Prometheus metrics."""
import time
...
class PrometheusMiddleware(BaseHTTPMiddleware):
    """
    Middleware for monitoring FastAPI applications using Prometheus metrics.

    This class provides mechanisms to track metrics such as the number of
    requests in progress, API response times, exceptions, and status codes.
    It is initialized with an ASGI app and an optional application name. All
    metrics are recorded and labeled with specific properties to allow
    detailed analysis and monitoring.

    :ivar app_name: Name of the application being monitored.
    :type app_name: str
    """
    def __init__(self, app: ASGIApp, app_name: str = 'fastapi-app') -> None:
        super().__init__(app)
        self.app_name = app_name
        INFO.labels(app_name=self.app_name.replace(' ', '').lower()).inc()
