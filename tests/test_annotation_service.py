...
from ina_ground_control.services.annotation_service import finish_annotation_crud, get_annotations_by_task_id_crud, create_annotation_crud, get_annotations_by_id_crud, skip_annotation_crud, udpate_annotation_result_crud, get_all_annotations_crud
...
def test_get_all_annotations_crud_filters(db_session: Session, skip=0, limit=100):
    ...
    # Filtred by email
    filtered_by_email = get_all_annotations_crud(db_session, user_email="user2.email@ina.fr",skip=0, limit=10)
    assert len(filtered_by_email) == 2
    assert filtered_by_email[0].user_email == created_annotation_2.user_email

    # Filtred by status
    filtered_by_status = get_all_annotations_crud(db_session, status=AnnotationStatus.IN_PROGRESS,skip=0, limit=10)
    ...
    filtered_by_project = get_all_annotations_crud(db_session, project_id=created_project.id,skip=0, limit=10)
    assert all(a.task.step.project_id == created_project.id for a in filtered_by_project)

    # Filtrage par step_id
    filtered_by_step = get_all_annotations_crud(db_session, step_id=created_step_1.id,skip=0, limit=10)
    ...
    filtered_by_created_at = get_all_annotations_crud(
        ...
        end_created_at=datetime(2025, 4, 17, 10, 0, 0),skip=0, limit=10
    )
    assert all(datetime(2025, 4, 17, 8, 0, 0) <= a.created_at <= datetime(2025, 4, 17, 10, 0, 0)
               for a in filtered_by_created_at)

    # Filtred by created_at newer
    filtered_by_created_at = get_all_annotations_crud(
        db_session,
        start_created_at=datetime(2025, 4, 17, 9, 0, 0),skip=0, limit=10
    )
    assert all(a.created_at >= datetime(2025, 4, 10, 9, 0, 0) for a in filtered_by_created_at)

    # Filtred by created_at older
    filtered_by_created_at = get_all_annotations_crud(
        db_session,
        end_created_at=datetime(2025, 4, 17, 9, 0, 0),skip=0, limit=10
    )
    assert all(a.created_at <= datetime(2025, 4, 10, 9, 0, 0) for a in filtered_by_created_at)
    ...
    filtered_by_updated_at = get_all_annotations_crud(
        ...
        end_updated_at=datetime(2025, 4, 17, 10, 0, 0),skip=0, limit=10
    )
    assert all(datetime(2025, 4, 17, 8, 0, 0) <= a.updated_at <= datetime(2025, 4, 17, 10, 0, 0)
               for a in filtered_by_updated_at)

    # Filtred by updated_at newer
    filtered_by_updated_at = get_all_annotations_crud(
        db_session,
        start_updated_at=datetime(2025, 4, 17, 9, 0, 0),skip=0, limit=10
    )
    assert all(a.updated_at >= datetime(2025, 4, 17, 9, 0, 0) for a in filtered_by_updated_at)

    # Filtred by updated_at older
    filtered_by_updated_at = get_all_annotations_crud(
        db_session,
        end_updated_at=datetime(2025, 4, 17, 9, 0, 0),skip=0, limit=10
    )
    assert all(a.updated_at <= datetime(2025, 4, 17, 9, 0, 0) for a in filtered_by_updated_at)
    ...
    filtered_by_validated_at = get_all_annotations_crud(
        ...
        end_validated_at=datetime(2025, 4, 17, 10, 0, 0),skip=0, limit=10
    )
    assert all(datetime(2025, 4, 17, 8, 0, 0) <= a.validated_at <= datetime(2025, 4, 17, 10, 0, 0)
               for a in filtered_by_validated_at)

    # Filtred by validated_at older
    filtered_by_validated_at = get_all_annotations_crud(
        db_session,
        end_validated_at=datetime(2025, 4, 17, 9, 0, 0),skip=0, limit=10
    )
    assert all(a.validated_at <= datetime(2025, 4, 10, 9, 0, 0) for a in filtered_by_validated_at)

    # Filtred by validated_at newer
    filtered_by_validated_at = get_all_annotations_crud(
        db_session,
        start_validated_at=datetime(2025, 4, 17, 9, 30, 0),skip=0, limit=10
    )
    assert all(a.validated_at >= datetime(2025, 4, 17, 9, 30, 0) for a in filtered_by_validated_at)
