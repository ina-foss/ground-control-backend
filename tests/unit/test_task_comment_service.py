...
from ina_ground_control.services.task_comment_service import (
    create_task_comment_crud,
    delete_task_comment_crud,
...
def test_get_task_comments(db_session: Session):
    ...
    created_task_comment_1 = create_task_comment_crud(
        TaskCommentCreate(**task_comment_data_1), db_session
    )
    created_task_comment_2 = create_task_comment_crud(
        TaskCommentCreate(**task_comment_data_2), db_session
    )
...
def test_get_task_comment_by_id(db_session: Session):
    created_task_comment = create_task_comment_crud(
        TaskCommentCreate(**task_comment_data), db_session
    )
...
def test_get_task_comment_by_task_id(db_session: Session):
    ...
    retrieved_task_comments = get_task_comment_by_task_id(
        db_session, created_task_comment_1.task_id
    )
...
def test_update_data_task_comment_crud(db_session: Session):
    ...
    update_task_comment_crud(
        created_task_comment.id,
        TaskCommentCreate(**updated_task_comment_data),
        db_session,
    )
    retrieved_updated_task_comment = get_task_comment_by_id(
        db_session, created_task_comment.id
    )

    assert retrieved_updated_task_comment is not None
    assert (
        retrieved_updated_task_comment.comment == updated_task_comment_data["comment"]
    )
    assert (
        retrieved_updated_task_comment.task_id == updated_task_comment_data["task_id"]
    )
